...
def without_prefix(parameter_name):
    return parameter_name.split('::', 1)[-1]


def _blank_param_cond(param):
    return not param['value'] or param['value'] == 'unset'


def _autogenerate_password_cond(param):
    # TODO(lsmola) Filter out SSL params for now. Once it will be autogenerated
    # in TripleO add it here too. Note: this will also affect how endpoints are
    # created
    key = param['name']
    return (param['hidden'] and _blank_param_cond(param) and
            without_prefix(key) not in SSL_HIDDEN_PARAMS and
            without_prefix(key) not in KEYSTONE_CERTIFICATE_PARAMS and
            key != 'SnmpdReadonlyUserPassword')


def _autogenerate_keystone_cert_cond(param):
    return (without_prefix(param['name']) in KEYSTONE_CERTIFICATE_PARAMS and
            _blank_param_cond(param))


def _autogenerate_neutron_control_plane_cond(param):
    return (without_prefix(param['name']) == 'NeutronControlPlaneID' and
            _blank_param_cond(param))
...
class Plan(base.APIResourceWrapper):
    ...
    def check_autogenerated_parameters(self, with_prefix=True):
        def key_format(key, with_prefix=True):
            if not with_prefix:
        ...
        for param in self.parameter_list():
            ...
            # Get all password like parameters
            if _autogenerate_password_cond(param):
                include_param = True

            # Get keystone certificates
            if _autogenerate_keystone_cert_cond(param):
                include_param = True

            # Get control plane id
            if _autogenerate_neutron_control_plane_cond(param):
                include_param = True

            if include_param:
                parameters[key_format(param['name'], with_prefix)] = param

        return parameters

    def _make_keystone_certificates(self, wanted_autogenerated_params):
        autogenerated_params = {}
        for cert_param in KEYSTONE_CERTIFICATE_PARAMS:
            if cert_param in wanted_autogenerated_params.keys():
        ...
        if generate_certificates:
            ...
            autogenerated_params['KeystoneSigningCertificate'] = (
                signing_cert_pem)
            autogenerated_params['KeystoneCACertificate'] = ca_cert_pem
            autogenerated_params['KeystoneSigningKey'] = signing_key_pem
        return autogenerated_params

    def make_autogenerated_parameters(self):
        wanted_autogenerated_params = self.check_autogenerated_parameters(
            with_prefix=False)

        # Generate keystone certificates
        autogenerated_params = self._make_keystone_certificates(
            wanted_autogenerated_params)

        # Generate passwords and control plane id
        for (key, param) in wanted_autogenerated_params.items():
            if _autogenerate_password_cond(param):
                autogenerated_params[key] = password_generator()
            elif _autogenerate_neutron_control_plane_cond(param):
                autogenerated_params[key] = neutron.network_list(
        ...
        wanted_prefixed_params = self.check_autogenerated_parameters(
            with_prefix=True)
        tuskar_params = {}

        for (key, param) in wanted_prefixed_params.items():
            tuskar_params[key] = autogenerated_params[without_prefix(key)]
